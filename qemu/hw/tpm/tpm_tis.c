...
static void tpm_volatile_save(void *opaque)
{
    qemu_log("---tpm_volatile_save---\n");
    uint32_t size = 12926 ;
    int i;
    unsigned char *buffer = NULL ;
    //unsigned char *buffer = opaque;
    TPM_RESULT rc;
    TPMVolatileState *tvs = opaque;
    buffer = malloc(size);
    if(buffer == NULL){
        abort();
    }

    ecall_TPM_VolatileAll_Store(global_eid, &rc, buffer);
    if (rc == TPM_SUCCESS) {
        qemu_log("TPMLIB_VolatileAll_Store success \n");
        qemu_log("buffer address1 :%p\n", &(*buffer));
        qemu_log("size1 :%d\n", size);

        /*for(i=0;i<size; i++)
            qemu_log("%x ",tvs->tpm_volatile[i]);
        qemu_log("\n ");*/

        tvs->length = size;
        memcpy(tvs->tpm_volatile, buffer, size);
        qemu_log("copy buffer to TPMVolatileState done \n");
    }
    else
        qemu_log("TPMLIB_VolatileAll_Store failed \n");
    //free(buffer);
}

static int tpm_volatile_load(void *opaque,int version_id)
{
    qemu_log("---tpm_volatile_load---\n");
    uint32_t size ;
    int i;
    ...
    size = tvs->length ;
    buffer = malloc(size);
    memcpy(buffer,tvs->tpm_volatile,size);
...
/*    for(i=0;i<size; i++)
    ...
    ecall_TPM_VolatileAll_Load(global_eid, &rc, buffer, size);
    if (rc == TPM_SUCCESS)
        qemu_log("TPMLIB_VolatileAll_Load success \n");
    else 
        qemu_log("TPMLIB_VolatileAll_Load failed \n");
...
{
    qemu_log("---tpm tis locality from addr---\n");
                                  ...
                                  unsigned size)
{
    ...
    case TPM_TIS_REG_ACCESS:
        qemu_log("tpm_tis_mmio_read() : TPM_TIS_REG_ACCESS\n");
    ...
    case TPM_TIS_REG_INT_ENABLE:
        qemu_log("tpm_tis_mmio_read() : TPM_TIS_REG_INT_ENABLE\n");
    ...
    case TPM_TIS_REG_STS:
        qemu_log("tpm_tis_mmio_read() : TPM_TIS_REG_STS\n");
    ...
    case TPM_TIS_REG_DATA_XFIFO ... TPM_TIS_REG_DATA_XFIFO_END:
        if (tis->active_locty == locty) {
		    ...
		    qemu_log("tpm_tis_mmio_read() : FIFO %02X\n",(uint8_t)v);
                    break;
                default:
    ...
    case TPM_TIS_REG_DID_VID:
        qemu_log("tpm_tis_mmio_read() : TPM_TIS_REG_DID_VID\n");
        val = (TPM_TIS_TPM_DID << 16) | TPM_TIS_TPM_VID;
        break;
    case TPM_TIS_REG_RID:
        qemu_log("tpm_tis_mmio_read() : TPM_TIS_REG_VID\n");
        val = TPM_TIS_TPM_RID;
        break;
#ifdef DEBUG_TIS
...
static void tpm_tis_mmio_write_intern(void *opaque, hwaddr addr,
                                      ...
                                      bool hw_access)
{
    qemu_log("---tpm tis mimo write intern---\n");
    TPMState *s = opaque;
    TPMTISEmuState *tis = &s->s.tis;
    ...
    switch (off) {
    case TPM_TIS_REG_ACCESS:
        qemu_log("tpm_tis_mmio_write_intern() : TPM_TIS_REG_ACCESS\n");
    ...
    case TPM_TIS_REG_STS:
        ...
        break;
    case TPM_TIS_REG_DATA_FIFO:
        qemu_log("tpm_tis_mmio_write_intern() : FIFO %02X\n\n");
    case TPM_TIS_REG_DATA_XFIFO ... TPM_TIS_REG_DATA_XFIFO_END:
        /* data fifo */
        if (tis->active_locty != locty) {
            break;
...
static const VMStateDescription vmstate_tpm_tis = {
    ...
    .pre_save = tpm_volatile_save , 
    .post_load = tpm_volatile_load ,
    .fields      = (VMStateField[]) {
    
	 VMSTATE_UINT32(length, TPMVolatileState),
	 VMSTATE_BUFFER(tpm_volatile, TPMVolatileState),	
   	 VMSTATE_END_OF_LIST()
    }
};
...
static void tpm_tis_class_init(ObjectClass *klass, void *data)
{
    ...
    qemu_log("... tpm_tis_class_init() ... \n");
	TPMVolatileState *tvs = g_malloc0(sizeof(TPMVolatileState));

    dc->realize = tpm_tis_realizefn;
    dc->props = tpm_tis_properties;
    dc->reset = tpm_tis_reset;
    //dc->vmsd  = &vmstate_tpm_tis;
    vmstate_register(NULL, 0, &vmstate_tpm_tis, tvs);//use a new register function to register tpm_tis
}
