...
TPM's persistent NVRAM data will be stored. Using the @code{qemu-img} tool, such an image can be created with a size of 500k.

Note that the @code{-tpmdev} id is @code{tpm0} and is referenced by in the @code{tpmdev=tpm0} in the @code{-device} option. Similarly, the @code{-drive}
id @code{tpm-nvram} is referenced by @code{nvram=tpm-nvram} in the @code{-tpmdev} option.
